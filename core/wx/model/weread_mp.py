...
class MpsWereadMP(MpsWeread):
    ...
    def _get_mp_articles_page(self, book_id: str, offset=0):
        # 通过宿主机Edge的CDP获取文章列表（绕过微信读书的x-wrpa动态签名）
        from core.config import cfg as _cfg
        import urllib.request as _urlreq
        agent_url = _cfg.get("weread.host_agent", "http://host.docker.internal:9000/fetch")
        try:
            body = json.dumps({"book_id": book_id, "offset": int(offset or 0)}).encode("utf-8")
            req = _urlreq.Request(agent_url, data=body,
            ...
            raise WereadMPAPIError(
                "host_agent_error",
                f"宿主机代理请求失败(请确认宿主机已启动host_weread_agent.py且Edge开着阅读器页): {exc}"
            ) from exc
        try:
            payload = json.loads(text)
        except ValueError as exc:
            raise WereadMPAPIError("invalid_json", "host agent returned non-JSON") from exc
        if not isinstance(payload, dict):
            raise WereadMPAPIError("invalid_response", "host agent response is not an object")
        _raise_response_error(payload)
        return payload
