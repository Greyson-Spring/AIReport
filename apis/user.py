...
async def get_user_info(current_user: dict = Depends(get_current_user_or_ak)):
    ...
    try:
        ...
        return success_response({
            ...
            "menu_permissions": json.loads(user.menu_permissions) if user.menu_permissions else [],
        })
...
async def add_user(
    ...
    try:
        ...
        # 创建新用户
        menu_perms = user_data.get("menu_permissions", [])
        new_user = DBUser(
            id=str(uuid4()),
            ...
            menu_permissions=json.dumps(menu_perms) if menu_perms else '',
...
async def update_user_info(
    ...
    try:
        ...
        if "menu_permissions" in update_data and current_user["role"] == "admin":
            menu_perms = update_data["menu_permissions"]
            user.menu_permissions = json.dumps(menu_perms) if isinstance(menu_perms, list) else ''
...
async def delete_user(
    ...
    try:
        ...
        if username == current_user["username"]:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=error_response(code=40003, message="不能删除自己的账号")
            )
...
@router.put("/{username}/reset-password", summary="管理员重置用户密码")
async def admin_reset_password(
    ...
    try:
        ...
        if len(new_password) < 6:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
                detail=error_response(code=40004, message="密码长度不能少于6位")
            )
        user = session.query(DBUser).filter(DBUser.username == username).first()
        if not user:
            raise HTTPException(
